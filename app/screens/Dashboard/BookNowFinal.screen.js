...
import {Divider, IndexPath, Layout, Modal, Icon} from '@ui-kitten/components';
...
const BookNowFinalScreen = ({
  ...
  const loadPackageData = async () => {
    ...
    packageData &&
      ...
      packageData.map(element => {
        element.details.itmArr = element.details.PackageItemSelected.filter(
          item => item.quantity,
    ...
    let packageFilterData =
      selectedLocationName &&
      selectedLocationName.map(ai => {
        packageData &&
          packageData.length > 0 &&
          packageData.map(item => {
            if (
              ...
              item.details.itmArr.includes(ai)
            ) {
    ...
    //       item.details.itmArr.includes(getlocationData.locationData.items.name)
    //     ) {
    //       return item;
    //     }
    //   });
  ...
  const _setLocationByItem = async (getObj, locData) => {
    ...
    const packageDetails = getObj.details.PackageItemSelected;
  ...
  const getAdditionalItemPrice = async (getObj, e) => {
    ...
    let temp_absenceDates = [];
    // if (absenceDates !== undefined) {
    //   temp_absenceDates = [];
    // }
    if (seasonalCheckbox) {
  ...
  const removeFromReducer = async name => {
    ...
    setsetsavecombinedLocationObjectState(getModifiedPkgData);
  ...
  const _calcSumOfAllLocation = async data => {
    ...
    selectedBookedLocationDataState &&
      ...
      selectedBookedLocationDataState.map(item => {
        item &&
          item.additem.map((e, i) => {
            var td = parseFloat(e.price) * parseFloat(e.value);
            if (item.name === data[0].Location) {
    ...
    dispatch(setFinalPrice(TotalValue));
    SetTotal(TotalValue);
  ...
  const _setLocationDataFunction = async (
    ...
    if (from === 'package') {
            ...
            for (let j = 0; j < resTemp.length; j++) {
              ...
              if (allResData.name === selectedItemLocation) {
                if (
                  ...
                  dataObj.packageDetails = packageDetails;
      ...
      console.log('getStoredUserLocData', getStoredUserLocData);
  ...
  const onBookNowClick = async () => {
    ...
    await dispatch(setBookingType('mapView'));
  ...
  const getPackageData = packageItem => {
    if (Object.keys(packageItem).length > 0) {
      const itemName = packageItem.items.name;
      const packageFilterData =
        packageData &&
        packageData.length > 0 &&
        packageData.filter(item => {
          if (
            ...
            item.details.PackageName &&
            itemName &&
            item.details.itmArr.includes(itemName)
          ) {
            return item;
          }
        });
    ...
    return (
      <>
        <BookFinalComponent
          ...
          loading={loading}
          language={language}
          allPackageData={getPackageData(item)}
          ...
          {finalLoading ? (
            ...
            <>
              <View>
                <TouchableOpacity
                  onPress={() => onClose()}
                  style={{alignItems: 'flex-end', margin: themeConfig.margin}}>
                  <Icon
                    style={styles.icon}
              ...
              <View
                ...
                <Typography category="s1">
                  {totalSum ? _checkIsNumberFloat(totalSum) : 0}€
                </Typography>
              </View>
              ...
              <View style={styles.boxStyle}>
                {summaryData && summaryData.length > 0 && (
                  <>
                    <Typography category="h3" style={{fontWeight: '500'}}>
                      {strings('summary')}
                    </Typography>
                    <SummaryComponent
...
export default React.memo(BookNowFinalScreen);
